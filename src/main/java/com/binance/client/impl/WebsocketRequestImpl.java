    ...
    WebsocketRequest<UserDataUpdateEvent> subscribeUserDataEvent(String listenKey,
        ...
        request.jsonParser = (jsonWrapper) -> {
            ...
            if(jsonWrapper.getString("e").equals("ACCOUNT_UPDATE")) {
                ...
                orderUpdate.setCommissionAsset(jsondata.getStringOrDefault("N", null)); // Commission Asset, will not push if no commission
                orderUpdate.setCommissionAmount(jsondata.getBigDecimalOrDefault("n", null)); // Commission, will not push if no commission
